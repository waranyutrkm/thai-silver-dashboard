...
✅ **ทำแล้ว (2026-09-08):** รับซื้อ/ขายออกเป็น **ราคาสุทธิต่อครั้ง** (ขายออก = ราคา×VAT×(1+คอมซื้อ+FX), รับซื้อ = ราคา×(1−คอมขาย−FX)) + มีคอลัมน์ "ค่าธรรมเนียมซื้อ/ขาย" + คิด fee ฝั่งขายแล้ว (feeSellPct) + tooltip แสดง raw→net · logic อยู่ใน `derive()` ใน `public/index.html`, ค่าธรรมเนียมใน `config.ts`

ยังเหลือ/ควรทำต่อ:
1. **จุดคุ้มทุน (break-even %)** — ราคาเงินต้องขึ้นกี่ % ถึงเท่าทุน round-trip (ซื้อสุทธิ vs รับซื้อสุทธิ) — ยังไม่ทำ
2. **ออกแบบ UI/UX ใหม่ได้ตามเหมาะสม** (`public/index.html` เป็น vanilla ไฟล์เดียว จะเปลี่ยนเป็น framework ก็ได้ แต่คงความ static เพื่อโฮสต์ Pages ฟรี)
3. เพิ่มร้าน/ช่องทาง: SNP/แสงนภา, YLG, Ausiris, MTS, KPT + เงิน 92.5% (ดูข้อจำกัดข้อ 10)
4. (ถ้าทำได้) contango จริงจากราคา 2 เดือนของ TFEX แทนค่าคงที่ (API มี seriesList หลายเดือน)
5. **ตัวเลขค่าธรรมเนียมยังเป็นค่าประมาณ** — ต้องหาของจริง (ข้อ 9)
...
## 12) TODO สรุป
- [x] แสดงค่าธรรมเนียมซื้อ-ขายรายครั้ง (ราคาสุทธิ + คอลัมน์ค่าธรรมเนียม) — เสร็จ
- [x] คิด fee ฝั่งขาย — เสร็จ
- [ ] จุดคุ้มทุน (break-even %) จาก round-trip
